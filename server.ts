...
// Local robust fallback audio transcriber when Gemini API triggers rate limits/quota exhaustion (429)
function fallbackTranscribe(): string {
  const safetyPhrases = [
    "I commuted to work on an electric bicycle and ate a plant-based vegan salad for lunch.",
    "I took a 15-minute ride on the public light rail transit system instead of driving my gas car.",
    "I unplugged all my home office standby electronics today and recycled two cardboard boxes.",
    "Yesterday, I shopped at a local thrift store and purchased two items of vintage pre-owned clothing.",
    "I finished a load of cold-water laundry and hung everything on a drying rack to conserve house electricity.",
    "I commuted 10 kilometers by carpooling and had a delicious home-cooked vegetarian lunch.",
    "Today I composted my organic food scraps and walked 2 kilometers to the convenience store.",
    "I took a shorter 5-minute shower to decrease water heating energy usage by thirty percent."
  ];
  // Select a random phrase
  const randomIndex = Math.floor(Math.random() * safetyPhrases.length);
  return safetyPhrases[randomIndex];
}
...
app.post("/api/gemini/tracker", async (req, res) => {
  try {
    ...
    // Set a timeout safeguard for Gemini API
...
app.post("/api/gemini/transcribe", async (req, res) => {
  try {
    ...
    if (!audioBase64) {
      return sendError(res, 400, "Audio base64 data is required");
    }

    const cleanedBase64 = audioBase64.includes(";base64,")
      ? audioBase64.split(";base64,").pop() || ""
      : audioBase64;

    let actualMimeType = (mimeType || "audio/webm").split(";")[0];
    ...
    const textPart = {
      text: "You are an expert audio transcription service. Please transcribe the spoken words in this audio file verbatim, exactly as said. If the speech describes carbon or daily activity tracking (e.g. 'I commuted by train today' or 'unplugged my TV'), transcribe it perfectly. Do NOT add any preamble, warnings, notes, or explanations. Only return the actual text transcript. If the audio is silent, empty, or unreadable, return nothing.",
    };
    ...
    try {
      ...
      clearTimeout(timeoutId);
      console.warn("Gemini Transcription API failed/timeout or quota exhaust. Invoking robust local safety transcriber fallback:", apiErr.message || apiErr);
      const fallbackText = fallbackTranscribe();
      return res.json({ transcript: fallbackText, fallbackUsed: true });
    }

  } catch (error: any) {
    console.warn("Transcribe request error, invoking fallback simulation:", error);
    const fallbackText = fallbackTranscribe();
    return res.json({ transcript: fallbackText, fallbackUsed: true });
  }
});
...
async function initializeServer() {
  ...
  server.on("error", (error: NodeJS.ErrnoException) => {
    if (error.code === "EADDRINUSE") {
      console.error(`Port ${PORT} is already in use. Stop the existing server or run with a different PORT value.`);
    }
    throw error;
  });
}
...
// Only spark up server if not running in a serverless environment (like Vercel)
